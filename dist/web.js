...
const schemaDir = path.join(repoRoot, "libs/bbb-dmx/schemas");
const schemaById = {
...
async function validateDocuments(payload) {
    ...
    for (const doc of payload.documents) {
        const file = typeof doc.name === "string" ? doc.name : "unnamed.json";
        const data = doc.data;
        const schema = isObject(data) && typeof data.schema === "string" ? data.schema : undefined;
        const fileName = schema ? schemaById[schema] : undefined;
        if (!schema || !fileName) {
            diagnostics.push({ file, severity: "error", message: `unsupported or missing schema '${schema ?? ""}'` });
            continue;
        }
        const schemaPath = path.join(schemaDir, fileName);
        const validator = ajv.getSchema(schemaPath);
        if (!validator) {
            diagnostics.push({ file, severity: "error", message: `internal error: schema not loaded for '${schema}'` });
            continue;
        }
        if (!validator(data)) {
            for (const error of validator.errors ?? []) {
                const pointer = error.instancePath.length > 0 ? error.instancePath : "/";
                diagnostics.push({ file, severity: "error", message: `${pointer} ${error.message ?? "schema validation failed"}` });
            }
        }
    }
...
.row { display:grid; grid-template-columns: repeat(6, minmax(70px, 1fr)); gap:6px; align-items:end; padding:8px 0; border-bottom:1px solid var(--line); }
...
function labeledInput(labelText, attrs = {}) { const outer=document.createElement('div'); const label=document.createElement('label'); const input=document.createElement('input'); label.textContent=labelText; for(const [key,value] of Object.entries(attrs)) { if(key === 'dataset') { for(const [dataKey,dataValue] of Object.entries(value)) input.dataset[dataKey] = String(dataValue); } else { input.setAttribute(key, String(value)); } } outer.append(label, input); return { outer, input }; }
function syncProfileFormFromState() { const p=selectedProfile(); const modeSelect=$('modeSelect'); clear(modeSelect); if(!p) { $('manufacturer').value=''; $('model').value=''; $('profileKey').value=''; $('profileJson').value=''; return; } $('manufacturer').value=p.manufacturer ?? ''; $('model').value=p.model ?? ''; $('profileKey').value=p.key ?? ''; for(const key of Object.keys(p.modes ?? {})) addOption(modeSelect, key); $('profileJson').value=pretty(p); }
function syncProfileStateFromForm() { const p=selectedProfile(); if(!p) return; p.manufacturer=$('manufacturer').value; p.model=$('model').value; p.key=$('profileKey').value; $('profileJson').value=pretty(p); renderProfiles(); renderPatch(); }
...
function updateFixture(i,row) { const f=state.patch.fixtures[i]; for(const input of row.querySelectorAll('input')) { const k=input.dataset.k; if(k==='id') f.id=input.value; if(k==='universe'||k==='address') f[k]=Number(input.value); if(k==='position'||k==='rotation') f[k]=parseTriple(input.value); } $('patchJson').value=pretty(state.patch); }
...
async function validate() { const docs=[]; for(const entry of state.profiles) docs.push({name:entry.suggestedFile ?? entry.profile.key+'.json', data:entry.profile}); if(state.patch) docs.push({name:'patch.json', data:state.patch}); status('Validating...'); const res=await fetch('/api/validate',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({documents:docs})}); const json=await res.json(); if(!res.ok) { status(json.error ?? 'validate failed', 'err'); return; } status(json.ok ? 'Schema validation passed.' : json.diagnostics.map(d=>d.file+': '+d.message).join('\n'), json.ok?'ok':'err'); }
...
$('applyProfile').onclick=()=>{ const p=parseJson('profileJson'); if(!state.profiles[state.selected]) state.profiles.push({profile:p,suggestedFile:(p.key??'profile')+'.json',source:'editor'}); else state.profiles[state.selected].profile=p; renderAll(); status('Applied profile JSON.', 'ok'); };
